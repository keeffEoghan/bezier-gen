const defaults = module.exports.defaults = {
    ...
    name: 'bezier',
    mix: 'mix',
    orders: Array(3).fill(0).map((v, i) => i+3),
...
function makeBezier(order = defaults.order, ...rest) {
    if(isNaN(order) && rest.length === 0) {
        const { order: o, type: t, name: n, mix: m } = order;
    ...
    const [type = defaults.type, name = defaults.name, mix = defaults.mix] = rest;
...
function makeBeziers(orders = defaults.orders, ...rest) {
    ...
    return pre+'\n'+
        types.map((t) => orders.map((o) => makeBezier(o, t, name, mix))
            .join('\n')).join('\n\n')+
        '\n'+suf+'\n';
}
